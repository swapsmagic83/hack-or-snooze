...
class StoryList {
  ...
  static async addStory( currentUser, {title,author,url}) {
    // UNIMPLEMENTED: complete this function!
   // alert("Inside add story: " + author + title + url);
    //Create new Story vai API
    let res = await axios({
      url: `${BASE_URL}/stories`,
      method: "POST",
      data:{
    ...
    let storyJson = res.data.story
  
    let story = new Story({storyId : storyJson.storyId, 
                            title : storyJson.title, 
                            author : storyJson.author, 
                            url : storyJson.url, 
                            username :storyJson.username, 
                            created : storyJson.createdAt
                          });
    return story
  }
}
async function markFavoriteStory(currentUser,storyId){
...
async function unmarkFavoriteStory(currentUser,storyId){
  let res= await axios({
    url : `${BASE_URL}/users/${currentUser.username}/favorites/${storyId}`,
    method: "DELETE",
    data:{
